...
class DataField_Dropzone extends DataField_Base{
	
	public static $properties = [
		"id", "label", "directory", "froalaID"
	];
	...
	public function getHTML($currentValue = ""){
		...
		<div class="form-group">
			...
			<div id="<?php echo($this->id); ?>" class="dropzone" style="overflow: auto" draggable="false">
				<!--Created with JS-->
			</div>
		</div>
	...
	function getJS($currentValue = ""){
		...
		<div id="dz_template">
			<div class="dz_preview col-xxs col-xs-6 col-sm-4 col-lg-3" draggable="true">
				<div class="thumbnail">
					<img data-dz-thumbnail />
				</div>
				<div class="dz_hover_details">
					<i class="fa fa-plus" aria-hidden="true" title="Insert"></i>
					<i class="fa fa-trash" aria-hidden="true" title="Delete" data-dz-remove></i>
				</div>
		...
		<script>
			...
			$("div#<?php echo($this->id); ?>").dropzone({
				...
				params: {reqType: 'imgUpload', folder: "<?php echo($this->directory);?>"},
				...
				init: function() {
					...
					$.get( "<?php echo(self::getPathToIncludes())?>/dropzone/file_ops.php", {reqType: 'imgList', folder: "<?php echo($this->directory);?>"}, function(data){
						...
						response.forEach(function(item, index){
							var mockFile = { name: item.name, size: item.fileSize, type: item.mime, url: item.url };
					...
					<?php if($this->froalaID != null && $this->froalaID != ""){
					?>
					.on("removedfile", function(file) {
						$.post("<?php echo(self::getPathToIncludes())?>/dropzone/file_ops.php", {reqType: 'imgDelete', src: file.url}); 
						try{
							$("#<?php echo($this->froalaID);?>").froalaEditor("image.remove", $("img[src='" + file.url + "']"));
						} catch (e) {}
						...
						file.previewTemplate.querySelector(".fa-plus").onclick = function(){
							$("#<?php echo($this->froalaID);?>").froalaEditor("image.insert", file.url);
						};
					})
					<?php
					}
			...
			console.log("<?php 
		$thisRel = substr(__DIR__, strlen($_SERVER['DOCUMENT_ROOT']));
		echo(self::getPathToIncludes()); ?>");
		</script>
