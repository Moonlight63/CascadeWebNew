...
$options['from_address']        = 'contact@cascadeap.com';                // Address the email is sent from
$options['from_name']           = 'Cascade Aerial Photography';           // Name to attach to the address
$options['to_addresses']        = array( 'contact@cascadeap.com' );       // To: addresse(s), add new array item for more
$options['bcc_addresses']       = array( 'contact@cascadeap.com' );       // BCC: addresse(s), add new array item for more
...
function getHtmlEmailContent()
{
	...
	// define content
	$content  = '<p><strong>' . $fdata['name']. ' has made an inquiry: ' . '</strong></p>';
	$content .= '<p>' . $fdata['message'] . '</p>'; 
	$content .= $fdata['name'] . '<br>';
	if (!empty($fdata['phone'])) { $content .= 'Tel: ' . $fdata['phone'] . '<br>'; }
	if (!empty($fdata['email'])) { $content .= 'Email: ' . $fdata['email']; }
...
function getPlainEmailContent()
{
	...
	// define content
	$content  = $fdata['name']. ' has made an inquiry: ' . PHP_EOL;
	$content .= $fdata['message'] . PHP_EOL;
	$content .= $fdata['name'] . PHP_EOL;
	if (!empty($fdata['phone'])) { $content .= 'Tel: ' . $fdata['phone'] . PHP_EOL; }
	if (!empty($fdata['email'])) { $content .= 'Email: ' . $fdata['email']; }
...
function sendEmailToSiteOwner()
{
	...
	$mail->Subject = $fdata['name'] . ' has made an online inquiry.';
