...
function showMessageBox ($msg, html) {
  $('html, body').animate({
    scrollTop: $("#contact").offset().top - 125
  }, 2000);
...
/**
 * Show a field has an error with a red border and an error message next to the label
 * @param  {object}  the field with the error
 * @param  {string}  the error text
 */
function showFieldError ($field, err) {
	$field.parent().addClass('has-error');
	$('label[for="' + $field[0].id + '"] > span').html(err);
}

/**
 * Removes the red border and error message next to the label
 * @param  {object}  the field with the error
 */
function removeFieldError ($field) {
	$field.parent().removeClass('has-error');
	$('label[for="' + $field[0].id + '"] > span').html('');
}
...
function getSuccessHtml (success) {
	...
	success_string +=     '<p style="font-size: 14px">'+success+'</p>';
	success_string += '</div>';
	...
	$('form.ucf').each(function(){
		...
		$form.submit(function(e){
			...
			if (validateTheForm($form)) {
				...
				$submit.html(translations.form.button_text[lang]).attr('disabled', 'disabled');
				...
				.done(function(data) {
					...
					if(data.success) {
						...
						$submit.html(translations.form.success.button_text[lang]).attr('disabled', 'disabled').removeClass('btn-default').addClass('btn-success');
